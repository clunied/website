 ...
 *
 * FROM_EMAIL must be on a domain verified in MailerSend.
 */
...
// Deliberately loose — MailerSend is the real authority on deliverability.
  ...
  async fetch(request, env) {
    ...
    if (!allowed) {
      return json(
        {
          error:
            "Forbidden. Only POST allowed from " + ALLOWED_ORIGINS.join(", "),
        },
        403,
        null,
      );
    ...
    if (!looksLikeEmail(email)) {
      return json({ error: "Invalid email address" }, 400, allowed);
    }
    ...
    const msRes = await fetch("https://api.mailersend.com/v1/email", {
      ...
      body: JSON.stringify({
        ...
        to: [{ email: toEmail }],
        // Lets the PT hit reply and reach the enquirer directly.
